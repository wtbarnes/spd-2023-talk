...
subtitle: "AAS SPD Meeting / Minneapolis, MN / 15 August 2023"
author:
- "Will Barnes"
- "Albert Shih"
- "Jake Parker"
...
csl: apj.csl
footer: "[wtbarnes.github.io/spd-2023-talk](https://wtbarnes.github.io/spd-2023-talk)"
...
from astropy.visualization import ImageNormalize, AsymmetricPercentileInterval, LogStretch, AsinhStretch, quantity_support
...
old_root_dir = pathlib.Path('../../sphere-workshop-2023-talk/')
observed_data_dir = old_root_dir / 'data' / 'observations' / 'ar_2020-11-09T18:00:00.000'
simulation_data_dir = old_root_dir / 'pipeline' / 'data'
...
with quantity_support():
    ...
    ax3.stairs(res_single['em_corona'].mean(axis=0),
    ...
    ax3.stairs(res_multi['em_corona'].mean(axis=0),
...
## EUV Observations Alone Not Sufficient

```{python}
#| eval: true
#| echo: false
try:
    all_lines = astropy.table.QTable.read(data_dir / 'selected_line_list.asdf')
    hot_lines = all_lines[all_lines['observed by MOXSI']]
except FileNotFoundError:
    # This cell computes the full line list for both the hot and cool lines
    line_list = astropy.table.QTable.read(old_root_dir / 'data' / 'moxsi-line-list.asdf')
    moxsi_lines = [
    ...
    hot_lines = astropy.table.QTable(astropy.table.vstack(
        [line_list[np.argmin(np.fabs(line_list['wavelength'] - w))] for w in moxsi_lines]
    ))
    hot_lines = hot_lines[['ion name', 'element', 'wavelength']]
    ...
    cool_lines = astropy.table.QTable(
        data=[[iname for iname,_ in winebarger_lines],
            [wave for _ ,wave in winebarger_lines]],
        names=['ion name', 'wavelength'],
        units={'wavelength': 'AA'},
    )
    cool_lines['element'] = list(map(lambda x: x.split()[0], cool_lines['ion name']))
    all_lines = astropy.table.vstack([hot_lines, cool_lines])
    all_lines['observed by MOXSI'] = all_lines['wavelength'] < 70*u.Angstrom

    # compute contribution functions
    temperature = 10**np.arange(5.5, 7.55, 0.05)*u.K
    density = 1e15*u.Unit('K cm-3') / temperature
    all_ions = np.unique(all_lines['ion name'])
    ions = {iname: fiasco.Ion(iname, temperature) for iname in all_ions}
    ...
    all_lines['contribution function'] = goft_col
    all_lines['contribution function temperature'] = [temperature,]
    all_lines['contribution function density'] = [density,]

    # compute intensities and errors
    error = 0.2
    all_lines['intensity'] = u.Quantity(
        [compute_intensity(row['contribution function'],
                           row['contribution function temperature'],
                           res_single['dem_corona'],
                           res_single['dem_temperature'])
        for row in all_lines]
    )
    all_lines['intensity error'] = error * all_lines['intensity']
    all_lines.write(data_dir / 'selected_line_list.asdf')

all_colors = wesanderson.color_palettes['The Grand Budapest Hotel'][0] + wesanderson.color_palettes['The Grand Budapest Hotel'][1]
element_colors = {name: color for name, color in zip(np.unique(all_lines['element']), all_colors)}
```
...
mcmc_cool_lines = []
for row in all_lines[all_lines['observed by MOXSI']==False]:
...
with quantity_support():
    for sample in dem_result_cool.samples:
        ax.stairs(sample.value*u.Unit('cm-5 K-1')*mcmc_temperature_bins.bin_widths,
                  edges=mcmc_temperature_bins.edges,
                  color='k',
                  alpha=0.35)
    ax.stairs(res_single['em_corona'].mean(axis=0),
              edges=res_single['dem_temperature_edges'],
              color=frequency_palette[1],
              lw=3)
    for row in all_lines[all_lines['observed by MOXSI']==False]:
        ax.plot(row['contribution function temperature'],
    ...
    ax.set_xscale("log")
    ax.set_yscale("log")
    ax.set_xlim(mcmc_temperature_bins.min * 0.9, mcmc_temperature_bins.max * 1.1)
    ax.set_ylim(1e25*u.cm**(-5), 1e30*u.cm**(-5))
    ax.set_xlabel('$T$ [K]')
    ax.set_ylabel('$\mathrm{EM}$ [cm$^{-5}$]')

plt.show()
```

::: aside
@winebarger_defining_2012
:::
...
for row in all_lines[all_lines['observed by MOXSI']]:
    _temp = row['contribution function temperature']
    _goft = row['contribution function']
    ax.plot(_temp, _goft, color=element_colors[row['element']], label=row['element'])
    ax.text(_temp[_goft.argmax()].value,
            _goft.max().value,
            row['ion name'],
...
::: {.aside}
Some in HXRs as well: FOXSI [@ishikawa_detection_2017], RHESSI [@ishikawa_hot_2019], NuSTAR (see talk by J. Duncan on Thursday, Session 406)
:::
...
::: {.r-stack}
![](static/wavelength-time-coverage.png){fig-align="center" width="75%"}
:::
::: {.aside}
...but some: SMM FCS [@del_zanna_elemental_2014], X123 [@caspi_new_2015], MaGIXS [@savage_first_2023]
:::
...
:::: {.columns}
::: {.column width="60%"}
- One of two instruments on the Cubesat Imaging X-ray Solar Spectrometer (**CubIXSS**)
- Full-disk slitless spectrograph ("overlappograph")
- Four XRT-like filtergram images 
- Spectral coverage from **1--72 Å (12.4--0.17 keV)**
...
:::
::: {.column width="40%"}
![](static/moxsi-pinhole-layout.png)
![](static/moxsi-grating.png)
:::
::::

::: {.notes}
1 year prime mission, will observe lots of active regions, well-positioned to place tighter constraints on amount of hot plasma in ARs
:::
...
for line in hot_lines[line_indices]:
...
```{python}
spec_cube_hf = read_data_cube(simulation_data_dir / 'p0.3' / 'spectra' / 'spec_cube_t1000.fits', hdu=1)
spec_cube_lf = read_data_cube(simulation_data_dir / 'p0.005' / 'spectra' / 'spec_cube_t1000.fits', hdu=1)

line_indices = [0, 9, 2, 3, 10]
fig = plt.figure(figsize=(14,6.8))
for i, row in enumerate(hot_lines[line_indices]):
...
```{python}
overlappograms_hf = [read_overlappogram(file) for file in sorted((pipeline_dir / f'p0.3' / 'overlappograms').glob('overlappogram_o*.fits'))]
overlappograms_lf = [read_overlappogram(file) for file in sorted((pipeline_dir / f'p0.005' / 'overlappograms').glob('overlappogram_o*.fits'))]
labels = ['High frequency', 'Low frequency']

sl = np.s_[0, 280:450, :]
norm = ImageNormalize(vmin=0, vmax=3e3, stretch=LogStretch())
fig = plt.figure(figsize=(14, 7))
axes = []
for i, (overlaps, label) in enumerate(zip([overlappograms_hf, overlappograms_lf], labels)):
    cropped_cube = stack_components(overlaps)[sl]
    ax = fig.add_subplot(3, 1, i+1, projection=cropped_cube.wcs)
    cropped_cube.plot(
        axes=ax,
        norm=norm,
        data_unit='ct',
...
cb = fig.colorbar(ax.get_images()[0], extend='max', ax=axes, orientation='horizontal', location='top', aspect=75)
cb.set_label('DN')
...
## HF versus LF: Where do they differ?
...
hf_summed.plot(axes=ax, ls='-', data_unit='ct', label='High frequency', color=frequency_palette[0])
lf_stacked = stack_components(overlappograms_lf)
lf_summed = lf_stacked[0].rebin((lf_stacked[0].data.shape[0], 1),operation=np.sum)[0]
lf_summed.plot(axes=ax, ls='-', data_unit='ct', label='Low frequency', color=frequency_palette[1])
ax.set_ylim(0, 5e4)
ax.set_xlim(980, 1600)
ax.set_ylabel('Summed Counts [DN]')
ax.coords[1].set_axislabel('HPC Latitude')
ax.legend(frameon=False)
...
def add_line_annotations(ax, cubes, colors, source_pos, line_table):
    for ol, color in zip(cubes, colors):
        line_pos, _, _ = ol.wcs.world_to_pixel(source_pos, line_table['wavelength'])
        for pos, row in zip(line_pos, line_table):
            text_ypos = 950
            ax.annotate(
...
max_order_index = 1
add_line_annotations(ax, overlappograms_hf[5:5+max_order_index], spectral_palette[1:], ar_center, hot_lines)
...
ax.coords[1].set_axislabel('HPC Latitude')
ax.legend(
    [Line2D([0], [0], color=spectral_palette[i], lw=2) for i in range(1,1+max_order_index)],
    [f'order={i}' for i in range(1,1+max_order_index)],
    frameon=False
)
plt.show()
```
...
## Inverting MOXSI Data

Usual emission measure distribution inversion can be done per pixel,

$$
I(c) = \mathrm{EM}(T)R(c,T)
$$

For slitless spectrograph data, each pixel includes contributions from *many spatial locations* and *many temperatures*,

$$
I(p) = \mathrm{EM}(x,T)R(p,x,T)
$$

::: {.aside}
@winebarger_unfolding_2019, @cheung_multi-component_2019, @savage_first_2023
:::

## Preliminary Results: Inverted vs. True EM

![](static/em-inverted-true-compare.png)

::: {.aside}
* Ridge: prioritize smoothness
* ElasticNet: balance both sparsity and smoothness

See poster 110.24 by A. Hochedez
:::

## Summary

- SXR holds key diagnostics for finding the 🚬🔫
- MOXSI will observe many ARs 1--72 Å range
...
- **Future:** Tuning hyperparameters to adjust sparsity, smoothness
- **Future:** Investigate wider heating parameter space
...
## Spectral Response in the SXR Range
...
## Some Observations in the SXR Range
...
for ol, color in zip(observed_components[5:7], spectral_palette[1:]):
    line_pos, _, _ = ol[:,330:400,900:].wcs.world_to_pixel(center_loc, hot_lines[line_indices]['wavelength'])
    for pos, row in zip(line_pos, hot_lines[line_indices]):
