...
subtitle: "AAS SPD Meeting / University of Minnesota, Minneapolis, MN / 15 August 2023"
author:
- "Will Barnes"
- "Albert Shih"
- "Jake Parker"
...
for ol, color in zip(overlappograms_hf[5:], spectral_palette[1:]):
    line_pos, _, _ = ol.wcs.world_to_pixel(ar_center, hot_lines['wavelength'])
    for pos, row in zip(line_pos, hot_lines):
        text_ypos = 950
        ax.annotate(
...
ax.legend(
    [Line2D([0], [0], color=spectral_palette[i], lw=2) for i in range(1,5)],
    [f'order={i}' for i in range(1,5)],
    frameon=False
)
...
## Recovered versus True Emission Measure
